...
// The API is what city24.lv's own front end calls. Presenting the same origin,
// referer and fetch metadata makes the request indistinguishable from that app —
// which matters because the bare request is refused from some hosts (it came back
// empty in 0.2s from a GitHub Actions runner while working fine from a home line).
...
async function collect(cfg, { log = () => {} } = {}) {
  ...
  for (let p = 1; p <= cfg.maxPages; p++) {
    ...
    let arr;
    try {
      arr = await getJSON(url, { headers: BROWSER_HEADERS });
    } catch (e) {
      // Never swallow this silently. When the API refuses (it blocks some
      // datacenter ranges outright), an empty result is indistinguishable
      // from 'no listings' unless the reason is printed.
      log(`    page ${p}: ${e && e.message ? e.message : e}`);
      break;
    }
